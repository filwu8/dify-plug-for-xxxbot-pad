...
class Dify(PluginBase):
    ...
    def __init__(self):
        ...
        try:
            ...
            # 加载黑名单配置
            self.blacklist_enable = plugin_config.get("blacklist_enable", False)
            self.blacklist_users = set(plugin_config.get("blacklist_users", []))
            self.blacklist_groups = set(plugin_config.get("blacklist_groups", []))
            
            if self.blacklist_enable:
                logger.info(f"黑名单功能已启用，用户黑名单: {len(self.blacklist_users)} 个，群组黑名单: {len(self.blacklist_groups)} 个")
                if self.blacklist_users:
                    logger.info(f"用户黑名单: {list(self.blacklist_users)}")
                if self.blacklist_groups:
                    logger.info(f"群组黑名单: {list(self.blacklist_groups)}")
            else:
                logger.info("黑名单功能未启用")
    ...
    def is_user_blacklisted(self, message: dict) -> bool:
        """检查用户是否在黑名单中"""
        if not self.blacklist_enable:
            return False
            
        user_wxid = message.get("SenderWxid", "")
        group_wxid = message.get("FromWxid", "")
        
        logger.debug(f"检查黑名单: 用户ID={user_wxid}, 群组ID={group_wxid}")
        logger.debug(f"当前黑名单用户: {list(self.blacklist_users)}")
        logger.debug(f"当前黑名单群组: {list(self.blacklist_groups)}")
        
        # 检查用户是否在黑名单中（支持多种格式）
        for blacklisted_id in self.blacklist_users:
            # 直接匹配
            if user_wxid == blacklisted_id:
                logger.info(f"用户 {user_wxid} 在黑名单中，消息被屏蔽")
                return True
            
            # 如果黑名单中是 wxid_ 格式，但实际ID不是，尝试匹配
            if blacklisted_id.startswith("wxid_") and user_wxid == blacklisted_id[5:]:
                logger.info(f"用户 {user_wxid} (对应 {blacklisted_id}) 在黑名单中，消息被屏蔽")
                return True
            
            # 如果实际ID是 wxid_ 格式，但黑名单中不是，尝试匹配
            if user_wxid.startswith("wxid_") and blacklisted_id == user_wxid[5:]:
                logger.info(f"用户 {user_wxid} (对应 {blacklisted_id}) 在黑名单中，消息被屏蔽")
                return True
            
        # 检查群组是否在黑名单中
        if message.get("IsGroup", False):
            for blacklisted_group in self.blacklist_groups:
                if group_wxid == blacklisted_group:
                    logger.info(f"群组 {group_wxid} 在黑名单中，消息被屏蔽")
                    return True
    ...
    async def handle_text(self, bot: WechatAPIClient, message: dict):
        ...
        # 检查黑名单
        if self.is_user_blacklisted(message):
            logger.info(f"黑名单用户 {message.get('SenderWxid', '')} 的消息被屏蔽")
            return
    ...
    async def handle_at(self, bot: WechatAPIClient, message: dict):
        ...
        # 检查黑名单
        if self.is_user_blacklisted(message):
            logger.info(f"黑名单用户 {message.get('SenderWxid', '')} 的@消息被屏蔽")
            return
    ...
    async def handle_quote(self, bot: WechatAPIClient, message: dict):
        ...
        # 检查黑名单
        if self.is_user_blacklisted(message):
            logger.info(f"黑名单用户 {message.get('SenderWxid', '')} 的引用消息被屏蔽")
            return False
    ...
    async def handle_voice(self, bot: WechatAPIClient, message: dict):
        ...
        # 检查黑名单
        if self.is_user_blacklisted(message):
            logger.info(f"黑名单用户 {message.get('SenderWxid', '')} 的语音消息被屏蔽")
            return
    ...
    async def handle_image(self, bot: WechatAPIClient, message: dict):
        ...
        # 检查黑名单
        if self.is_user_blacklisted(message):
            logger.info(f"黑名单用户 {message.get('SenderWxid', '')} 的图片消息被屏蔽")
            return
    ...
    async def handle_xml_quote(self, bot: WechatAPIClient, message: dict):
        ...
        # 检查黑名单
        if self.is_user_blacklisted(message):
            logger.info(f"黑名单用户 {message.get('SenderWxid', '')} 的XML引用消息被屏蔽")
            return False
    ...
    async def handle_xml_file(self, bot: WechatAPIClient, message: dict):
        ...
        # 检查黑名单
        if self.is_user_blacklisted(message):
            logger.info(f"黑名单用户 {message.get('SenderWxid', '')} 的XML文件消息被屏蔽")
            return False
    ...
    async def handle_file(self, bot: WechatAPIClient, message: dict):
        ...
        # 检查黑名单
        if self.is_user_blacklisted(message):
            logger.info(f"黑名单用户 {message.get('SenderWxid', '')} 的文件消息被屏蔽")
            return
